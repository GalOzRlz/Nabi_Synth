...
pub fn modal_plucked_string(state: &SharedMidiState) -> Box<dyn AudioUnit> {
    ...
    let modes = (mul(1.0) >> sine() * (gate.clone() >> adsr_live(0.002, 0.5/0.3, 0.0, synth_adsr.release))
        & (mul(2.001) >> sine() * a1) * (gate.clone() >> adsr_live(0.0001, 0.5/0.8, 0.0, synth_adsr.release))
        & (mul(3.004) >> sine() * a2) * (gate.clone() >> adsr_live(0.0001, 0.5/1.5, 0.0, synth_adsr.release))
        & (mul(4.010) >> sine() * a3) * (gate.clone() >> adsr_live(0.0001, 0.5/2.5, 0.0, synth_adsr.release))
        & (mul(5.020) >> sine() * a4) * (gate.clone() >> adsr_live(0.0001, 0.5/4.0, 0.0, synth_adsr.release))
        & (mul(6.035) >> sine() * a5) * (gate.clone() >> adsr_live(0.0001, 0.5/6.0, 0.0, synth_adsr.release))
        & (mul(7.055) >> sine() * a6) * (gate.clone() >> adsr_live(0.0001, 0.5/8.5, 0.0, synth_adsr.release))
        & (mul(8.080) >> sine() * a7) * (gate.clone() >> adsr_live(0.0001, 0.5/12.0, 0.0, synth_adsr.release))
        >> highpass_hz(40.0, 0.6)
        >> lowpass_hz(4000.0, 0.7))
        >> dcblock::<f64>() * 0.5 >> clip();

    let tone = modes >> (pass() ^ (highpass_hz(100.0, 0.7) * 0.10)) >> join::<U2>();


    let synth = Box::new(
        tone >> highpass_hz(30.0, 0.7) >> dcblock::<f64>() >> limiter(0.002, 0.12),
    );
...
pub fn modal_bell(state: &SharedMidiState) -> Box<dyn AudioUnit> {
    ...
    let modes = (mul(1.000) >> sine() * (gate.clone() >> adsr_live(0.002, 0.6/0.5, 0.0, synth_adsr.release))
        & (mul(2.143) >> sine() * a1) * (gate.clone() >> adsr_live(0.002, 0.6/3.0, 0.0, synth_adsr.release))
        & (mul(3.413) >> sine() * a2) * (gate.clone() >> adsr_live(0.002, 0.6/6.0, 0.0, synth_adsr.release))
        & (mul(4.090) >> sine() * a3) * (gate.clone() >> adsr_live(0.002, 0.6/10.0, 0.0, synth_adsr.release))
        & (mul(5.190) >> sine() * a4) * (gate.clone() >> adsr_live(0.002, 0.6/15.0, 0.0, synth_adsr.release))
        & (mul(6.250) >> sine() * a5) * (gate.clone() >> adsr_live(0.002, 0.6/20.0, 0.0, synth_adsr.release))
...
pub fn modal_struck_bar(state: &SharedMidiState) -> Box<dyn AudioUnit> {
    ...
    let synth = Box::new(
        tone >> body * db_amp(-4.0) >> highpass_hz(30.0, 0.7) >> dcblock::<f64>() >> limiter(0.002, 0.12),
    );
...
pub fn modal_membrane(state: &SharedMidiState) -> Box<dyn AudioUnit> {
    ...
    let modes = (mul(1.000) >> sine() * (gate.clone() >> adsr_live(0.002, decay_base/0.2, 0.0, synth_adsr.release))
        & (mul(1.593) >> sine() * a1) * (gate.clone() >> adsr_live(0.002, decay_base/0.8, 0.0, synth_adsr.release))
        & (mul(2.135) >> sine() * a2) * (gate.clone() >> adsr_live(0.002, decay_base/1.8, 0.0, synth_adsr.release))
        & (mul(2.296) >> sine() * a3) * (gate.clone() >> adsr_live(0.002, decay_base/3.0, 0.0, synth_adsr.release))
        & (mul(2.917) >> sine() * a4) * (gate.clone() >> adsr_live(0.002, decay_base/5.0, 0.0, synth_adsr.release))
...
pub fn breathy(state: &SharedMidiState) -> Box<dyn AudioUnit> {
    ...
    let pipe2 = (pipe1.clone() | pitch.clone() * constant(2.015) |  constant(75.0))  >> resonator() * constant(0.6);
    let pipe3 = (pipe2.clone() | pitch.clone() * constant(3.120) |  constant(75.0))  >> resonator() * constant(0.2);
    let pipe4 = (pipe2.clone() | pitch.clone() * constant(4.215) |  constant(75.0))  >> resonator() * constant(0.10);
    let pipe5 = (pipe2.clone() | pitch.clone() * constant(5.320) |  constant(65.0))  >> resonator() * constant(0.05);
    // Post‑processing (same as other instruments)
    let processed = ((attack & pipe1 & pipe2 & pipe3 & pipe4 & pipe5) * constant(0.5) | pitch.clone() | constant(0.5))
        >> highpass()
...
pub fn dirty_guitar(state: &SharedMidiState) -> Box<dyn AudioUnit> {
    let adsr = Adsr {
        attack: 0.005,
        decay: 0.8,
        sustain: 1.0,
    ...
    let pitch = base_pitch * (constant(1.0) + lfo);
    let gate = state.control_var().clone();
    let mix = (pitch  | gate | constant(0.0))
        >> pluck_comb_string()
        >> lowpass_hz(9000.0, 0.5);
    state.assemble_pitched_sound(Box::new(mix), adsr.boxed(state))
}